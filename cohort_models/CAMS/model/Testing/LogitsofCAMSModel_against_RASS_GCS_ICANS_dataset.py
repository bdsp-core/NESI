...
CAMS_model_trained = Score_model_load(CAMS_model_path)
...
RASS_test_CORN_logits = get_corn_model_output(CAMS_model_trained, X_tst_data_global)    
print("\nCORN model logits shape:", RASS_test_CORN_logits.shape)
print("True labels shape:", np.array(Yraw_tst_global).shape)

RASS_data_CAMS3cls_probs, RASS_data_CAMS3cls_logits = \
    corn_to_class_probs(RASS_test_CORN_logits, num_classes=3)


print("\nClass probs shape:", RASS_data_CAMS3cls_probs.shape)
print("Class logits shape:", RASS_data_CAMS3cls_logits.shape)
...
GCS_test_CORN_logits = get_corn_model_output(CAMS_model_trained, X_tst_data_global)    
print("\nCORN model logits shape:", GCS_test_CORN_logits.shape)
print("True labels shape:", np.array(Yraw_tst_global).shape)

GCS_data_CAMS3cls_probs, GCS_data_CAMS3cls_logits = \
    corn_to_class_probs(GCS_test_CORN_logits, num_classes=3)


print("\nClass probs shape:", GCS_data_CAMS3cls_probs.shape)
print("Class logits shape:", GCS_data_CAMS3cls_logits.shape)


# Clean the dataset (multiplier=3.0 keeps some outliers, drops the extreme ones)
GCS_logits_clean, GCS_y_true_clean = clean_logit_outliers(
    GCS_data_CAMS3cls_logits, 
    Yraw_tst_global, 
    iqr_multiplier=3.0
)
...
# # **Test on ICANS dataset**

# In[11]:


print('+'*80)
print('        Working on ICANS Dataset')
print('+'*80)
df_test_ICANS  = get_test_metedata_cohort(df_ICANS_metadata)

X_tst_data_global, _, Yraw_tst_global, _, tst_dataset_names_global, _ = morgoth_10minfea_matrix(df_test_ICANS)

ICANS_test_CORN_logits = get_corn_model_output(CAMS_model_trained, X_tst_data_global)    
print("\nCORN model logits shape:", ICANS_test_CORN_logits.shape)
print("True labels shape:", np.array(Yraw_tst_global).shape)

ICANS_data_ICANS3cls_probs, ICANS_data_ICANS3cls_logits = \
    corn_to_class_probs(ICANS_test_CORN_logits, num_classes=3)


print("\nClass probs shape:", ICANS_data_ICANS3cls_probs.shape)
print("Class logits shape:", ICANS_data_ICANS3cls_logits.shape)


# Clean the dataset (multiplier=3.0 keeps some outliers, drops the extreme ones)
ICANS_logits_clean, ICANS_y_true_clean = clean_logit_outliers(
    ICANS_data_ICANS3cls_logits, 
    Yraw_tst_global, 
    iqr_multiplier=3.0
)
...
# # # **Consolidate everything in same figure**
...
def plot_all_scales_boxplot_grid(
    ...
    for col_idx, scale_name in enumerate(scales_order):
        ...
        for row_idx in range(num_rows):
            ...
            for box in bp['boxes']:
                box.set(facecolor=current_color, edgecolor='black', linewidth=0.7)
...
dataset_payload = {
    ...
    'ICANS': {
        'logits': ICANS_logits_clean,
        'y_true': ICANS_y_true_clean,
        'xlabel': 'True ICANS scale'
    }
}


CAMS_model_test_saving_path = ICANS_ROOT / "cohort_models" / "CAMS" / "Figures" / "LogitsAgainstOtherScalesvsCAMSModel.png"
...
plot_all_scales_boxplot_grid(
    ...
    fontsize=8,
    dpi=210,
    save_path=CAMS_model_test_saving_path
)
