...
def morgoth_10minfea_matrix_stat_for(data_frame):
    ...
    train_labels=data_frame['CAMS_SF'].values .flatten()  
...
def RESNet_kfold_stratified_group(df_CAMS_metadata, base_model_dir , dl_model, n_splits=5, bs=32):
    ...
    df_subjects = df_CAMS_metadata[['BDSPPatientID','CAMS_SF']].drop_duplicates()
    unique_studies = df_subjects['BDSPPatientID'].values
    labels = df_subjects['CAMS_SF'].values
    ...
    for fold, (train_idx, test_idx) in enumerate(sgkf.split(unique_studies, labels, groups=groups)):
        ...
        df_ICANS_train = df_CAMS_metadata[df_CAMS_metadata['Split']=='Train'].reset_index(drop=True)
        df_ICANS_test  = df_CAMS_metadata[df_CAMS_metadata['Split']=='Test'].reset_index(drop=True)
...
model_normal = MORGOTH_ResNet1D_BiLSTM_TimeShuffle(num_features=17, num_classes=df_CAMS_metadata['CAMS_SF'].nunique(), time_shuffle=False)
base_model_dir = CAMS_ROOT / "model" / "ModelCheckpointsNew" / "8class" / "RESNET_notimeshift"
...
# Time-shuffled model
model_shuffle = MORGOTH_ResNet1D_BiLSTM_TimeShuffle(num_features=17, num_classes=df_CAMS_metadata['CAMS_SF'].nunique(), time_shuffle=True)
...
# -------------------------------RESNET only GAP----------------------
model_onlygap = MORGOTH_ResNet1D_onlyGAP_CORAL(num_features=17, num_classes=df_CAMS_metadata['CAMS_SF'].nunique())
...
# # Save to pickle file
# with open('/home/ayush/Desktop/CAM-S_dataset/RESULTS/RESNET_5foldOnlyGap_Results_8class.pkl', 'wb') as f:
...
# # Save to pickle file
# with open('/home/ayush/Desktop/CAM-S_dataset/RESULTS/RESNET_5foldNOSHUFFLE_Results_8class.pkl', 'wb') as f:
