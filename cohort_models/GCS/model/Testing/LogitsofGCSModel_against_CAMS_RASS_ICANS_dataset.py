...
def plot_corn_logits_boxplot(
    ...
    for i in range(num_classes):
        ...
        for box in bp['boxes']:
            box.set(facecolor=current_color, edgecolor='black', linewidth=1.2)
...
GCS_model_trained = Score_model_load(GCS_model_path)
...
CAMS_test_CORN_logits = get_corn_model_output(GCS_model_trained, X_tst_data_global)    
print("\nCORN model logits shape:", CAMS_test_CORN_logits.shape)
print("True labels shape:", np.array(Yraw_tst_global).shape)

CAMS_data_CAMS3cls_probs, CAMS_data_CAMS3cls_logits = \
    corn_to_class_probs(CAMS_test_CORN_logits, num_classes=3)


print("\nClass probs shape:", CAMS_data_CAMS3cls_probs.shape)
print("Class logits shape:", CAMS_data_CAMS3cls_logits.shape)
...
ICANS_test_CORN_logits = get_corn_model_output(GCS_model_trained, X_tst_data_global)    
...
RASS_test_CORN_logits = get_corn_model_output(GCS_model_trained, X_tst_data_global)    
...
GCS_model_test_saving_path = GCS_ROOT / "cohort_models" / "GCS" / "Figures" / "LogitsAgainstOtherScalesvsGCSModel.png"
...
plot_all_scales_boxplot_grid(
    ...
    fontsize=8,
    dpi=210,
    save_path=GCS_model_test_saving_path
)
