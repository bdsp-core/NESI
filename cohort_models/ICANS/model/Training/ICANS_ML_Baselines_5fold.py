...
def morgoth_10minfea_matrix_stat_for(data_frame, statistic):
    ...
    Args:
        data_frame: pd.DataFrame with column 'MorgothOutputFilename' and 'ICANS_grouped'
...
def LR_kfold_stratified_group(df_ICANS_metadata, feature_stat, base_model_dir, n_splits=5):
    """
    Stratified group k-fold cross-validation (~15% test per fold) 
    ensuring no subject (StudyID) appears in both train and test.
    
    Args:
        df_ICANS_metadata: DataFrame with at least ['StudyID', 'ICANS_grouped']
    ...
    df_subjects = df_ICANS_metadata[['StudyID','ICANS_grouped']].drop_duplicates()
    unique_studies = df_subjects['StudyID'].values
    labels = df_subjects['ICANS_grouped'].values
    groups = df_subjects['StudyID'].values  # important to prevent overlap
    ...
    for fold, (train_idx, test_idx) in enumerate(sgkf.split(unique_studies, labels, groups=groups)):
        ...
        df_ICANS_metadata['Split'] = df_ICANS_metadata['StudyID'].apply(
            lambda x: 'Train' if x in train_subs else 'Test'
        )
        
        df_ICANS_train = df_ICANS_metadata[df_ICANS_metadata['Split']=='Train'].reset_index(drop=True)
        df_ICANS_test  = df_ICANS_metadata[df_ICANS_metadata['Split']=='Test'].reset_index(drop=True)
        
        # Feature extraction
        X_train_fea, Y_train = morgoth_10minfea_matrix_stat_for(df_ICANS_train, feature_stat)
        X_test_fea, Y_test   = morgoth_10minfea_matrix_stat_for(df_ICANS_test, feature_stat)
        ...
        fold_results.append({
            ...
            'True_grouped_ICANS':Y_test,
            'Predictions_grouped_ICANS':Y_pred
        })
...
def SVM_kfold_stratified_group(df_ICANS_metadata, feature_stat, base_model_dir, n_splits=5):
    ...
    unique_studies = df_subjects['StudyID'].values
    labels = df_subjects['ICANS_grouped'].values
    groups = df_subjects['StudyID'].values
    ...
    for fold, (train_idx, test_idx) in enumerate(sgkf.split(unique_studies, labels, groups=groups)):
        ...
        df_train = df_ICANS_metadata[df_ICANS_metadata['Split']=='Train'].reset_index(drop=True)
        df_test  = df_ICANS_metadata[df_ICANS_metadata['Split']=='Test'].reset_index(drop=True)
...
def KNN_kfold_stratified_group(df_ICANS_metadata, feature_stat, base_model_dir, n_splits=5, n_neighbors=5):
...
print('Total Unique subject ==> '+str(df_ICANS_metadata['StudyID'].nunique()))
...
feature_stat1 = 'mean'
base_model_dir = ICANS_ROOT / "model" / "ModelCheckpointsNew" / "LR_ICANS"

LR_results_5fld_mean = LR_kfold_stratified_group(df_ICANS_metadata, feature_stat1, base_model_dir, n_splits=5)
...
base_model_dir = ICANS_ROOT / "model" / "ModelCheckpointsNew" / "LR_ICANS"
LR_results_5fld_median = LR_kfold_stratified_group(df_ICANS_metadata, feature_stat2, base_model_dir, n_splits=5)
...
LR_results_5fld_covar = LR_kfold_stratified_group(df_ICANS_metadata, feature_stat3, base_model_dir, n_splits=5)
...
base_model_dir = ICANS_ROOT / "model" / "ModelCheckpointsNew" / "SVM_ICANS"

SVM_results_5fld_mean = SVM_kfold_stratified_group(df_ICANS_metadata, feature_stat1, base_model_dir, n_splits=5)
...
SVM_results_5fld_median = SVM_kfold_stratified_group(df_ICANS_metadata, feature_stat2, base_model_dir, n_splits=5)
...
SVM_results_5fld_covar = SVM_kfold_stratified_group(df_ICANS_metadata, feature_stat3, base_model_dir, n_splits=5)
...
base_model_dir = ICANS_ROOT / "model" / "ModelCheckpointsNew" / "KNN_ICANS"

KNN_results_5fld_mean = KNN_kfold_stratified_group(df_ICANS_metadata, feature_stat1, base_model_dir, n_splits=5)
...
KNN_results_5fld_median = KNN_kfold_stratified_group(df_ICANS_metadata, feature_stat2, base_model_dir, n_splits=5)
...
KNN_results_5fld_covar = KNN_kfold_stratified_group(df_ICANS_metadata, feature_stat3, base_model_dir, n_splits=5)
...
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/KNN_5fld_mean_Results.pkl'
# )
# save_to_pickle(
#     KNN_results_5fld_median,
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/KNN_5fld_median_Results.pkl'
# )
# save_to_pickle(
#     KNN_results_5fld_covar,
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/KNN_5fld_covar_Results.pkl'
# )
...
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/LR_5fld_mean_Results.pkl'
# )
# save_to_pickle(
#     LR_results_5fld_median,
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/LR_5fld_median_Results.pkl'
# )
# save_to_pickle(
#     LR_results_5fld_covar,
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/LR_5fld_covar_Results.pkl'
# )
...
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/SVM_5fld_mean_Results.pkl'
# )
# save_to_pickle(
#     SVM_results_5fld_median,
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/SVM_5fld_median_Results.pkl'
# )
# save_to_pickle(
#     SVM_results_5fld_covar,
#     '/home/ayush/Desktop/ICANS_dataset/RESULTS/SVM_5fld_covar_Results.pkl'
# )
