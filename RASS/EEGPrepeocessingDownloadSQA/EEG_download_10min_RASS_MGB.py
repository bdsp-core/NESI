...
# Extrcat 10 in EEG
def extrcat_10min_EEG_with_RASS(edf_path, df_sub, fs):
    # Load EEG data
    ...
    # Round timestamps to the nearest second (ignore micro/nanoseconds)
    time_index = time_index.floor('s')

    # RASS times and values
    rass_times = pd.to_datetime(df_sub['RASSRecordedDTS'].dropna().tolist()).floor('s')
    rass_values = df_sub['R PHS IP RASS'].dropna().astype(int).tolist()
    # Containers for extracted EEG segments and labels
    X_segments, Y_labels, valid_rass_times = [], [], []

    for t, val in zip(rass_times, rass_values):
        # define window: 10 min before t
        start_time = t - pd.Timedelta(minutes=10)
        end_time = t
        # print('EEG cut out==> Start time: ' + str(start_time) + ' End time: ' + str(end_time))
        
        # check if window lies within EEG recording
        if start_time >= EEG_begin and end_time <= EEG_end:
            # find indices corresponding to start_time and end_time
            idx_start = np.searchsorted(time_index, start_time)
            idx_end = np.searchsorted(time_index, end_time)
    
            # print(f"Start index: {idx_start}, End index: {idx_end}, samples in window: {idx_end-idx_start}")
            
            # ensure enough samples exist
            if idx_end - idx_start == samples_per_window:
                EEG_segment = EEG[:, idx_start:idx_end]
            else:
                # if off by one due to rounding
                print('Activated phase-2')
                EEG_segment = EEG[:, idx_end - samples_per_window:idx_end]
            
            X_segments.append(EEG_segment)
            Y_labels.append(val)
            valid_rass_times.append(t)
...
AWS_keys_path = RASS_ROOT / "Cohort" / "RASSEEG_download_AWS.txt"
with open(AWS_keys_path, "r") as f:
    all_paths = [line.strip() for line in f if line.strip()]



def download_extract_cleanup(all_paths, RASS_harvard_metadata, fs=200):
    """
    ...
    RASS_harvard_metadata : pd.DataFrame
    ...
    for cmd in tqdm(all_paths[:], desc="Processing EEG downloads"): #0:100
        ...
        for edf_path in edf_files:
            ...
            # ------------------------------
            df_sub = RASS_harvard_metadata[
                (RASS_harvard_metadata["BDSPPatientID"] == BDSPPatientID)
                & (RASS_harvard_metadata["SessionID"] == session_id)
            ]
            ...
            try:
                EEG_10min_windows, RASS_recordings = extrcat_10min_EEG_with_RASS(edf_path, df_sub, fs)
            except Exception as e:
            ...
            sio.savemat(mat_filename, {
                ...
                "RASS_recordings": RASS_recordings
            })
...
download_extract_cleanup(all_paths, RASS_harvard_metadata, fs=200)