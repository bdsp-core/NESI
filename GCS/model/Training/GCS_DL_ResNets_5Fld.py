...
# # **Feature engineering an training loop helpers**
...
def RESNet_kfold_stratified_group(df_GCS_metadata, base_model_dir , dl_model, n_splits=5, bs=32):
    """
    ...
    for fold, (train_idx, test_idx) in enumerate(sgkf.split(unique_studies, labels, groups=groups)):
        ...
        df_ICANS_train = df_GCS_metadata[df_GCS_metadata['Split']=='Train'].reset_index(drop=True)
        df_ICANS_test  = df_GCS_metadata[df_GCS_metadata['Split']=='Test'].reset_index(drop=True)
...
model_normal = MORGOTH_ResNet1D_BiLSTM_TimeShuffle(num_features=17, 
                                                   num_classes=df_GCS_metadata['GCS_grouped'].nunique(), 
                                                   time_shuffle=False)
base_model_dir = GCS_ROOT / "model" / "ModelCheckpointsNew" / "RESNET_notimeshuffle"

RESNet_results_notimeshuffle = RESNet_kfold_stratified_group(df_GCS_metadata, base_model_dir, model_normal,
                                                        n_splits=5, bs=128)
...
model_shuffle = MORGOTH_ResNet1D_BiLSTM_TimeShuffle(num_features=17, 
                                                    num_classes=df_GCS_metadata['GCS_grouped'].nunique(), 
                                                    time_shuffle=True)
base_model_dir = GCS_ROOT / "model" / "ModelCheckpointsNew" / "RESNET_TIMESHIFT"

RESNet_results_withtimeshuffle = RESNet_kfold_stratified_group(df_GCS_metadata, base_model_dir, model_shuffle, 
                                                        n_splits=5, bs=128)
...
# -------------------------------RESNET only GAP----------------------
model_onlygap = MORGOTH_ResNet1D_onlyGAP_CORAL(num_features=17, num_classes=df_GCS_metadata['GCS_grouped'].nunique())

base_model_dir = GCS_ROOT / "model" / "ModelCheckpointsNew" / "RESNET_GAPonly"

RESNet_results_gaponly = RESNet_kfold_stratified_group(df_GCS_metadata, base_model_dir, model_onlygap, 
                                                        n_splits=5, bs=32)


# # **Save 5 fold results in your desired place**
...
# # Save to pickle file
# with open('/home/ayush/Desktop/MGB_GCS_EEGs/Model_checkpoints_new/Ordinal/Results_5fld/RESNET_5foldOnlyGap_GCS_Results.pkl', 'wb') as f:
...
# # Save to pickle file
# with open('/home/ayush/Desktop/MGB_GCS_EEGs/Model_checkpoints_new/Ordinal/Results_5fld/RESNET_5foldNOSHUFFLE_GCS_Results.pkl', 'wb') as f:
...
# # Save to pickle file
# with open('/home/ayush/Desktop/MGB_GCS_EEGs/Model_checkpoints_new/Ordinal/Results_5fld/RESNET_5foldTIMESHUFFLE_GCS_Results.pkl', 'wb') as f:
