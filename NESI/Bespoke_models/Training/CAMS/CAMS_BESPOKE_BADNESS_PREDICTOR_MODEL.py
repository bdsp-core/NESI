...
#  - **Good neurological state (0):** CAMS(0, 1)
#  - **Moderate state (1):** RASS (2-5)
#  - **Bad state (2):**  RASS(6-7)
...
df_train = df_all_metadata[(df_all_metadata['Split'] == 'Train') & (df_all_metadata['Dataset'] == 'CAMS')].reset_index(drop=True)
df_val   = df_all_metadata[(df_all_metadata['Split'] == 'Val') & (df_all_metadata['Dataset'] == 'CAMS')].reset_index(drop=True)
df_test  = df_all_metadata[(df_all_metadata['Split'] == 'Test') & (df_all_metadata['Dataset'] == 'CAMS')].reset_index(drop=True)
...
def morgoth_output_file_location(data_group):
    base_paths = {
        ...
        "CAMS": GCS_ROOT / "cohort_models" / "CAMS" / "MorgothActivations",
...
model_bespoke_bdness_path = NSEI_ROOT / "Bespoke_models"/ "ModelCheckpoints_NEW" / "BespokeBadnessCheckpoint" / "cohort_models" / "CAMS" / "CAMS_Bespoke_Badness_Bet_model.pth"
...
plot_score_distribution_by_raw(
    ...
    y_raw=Yraw_train,
    dataset_name="CAMS",
    title="CAMS Train Set Score Distribution"
)
