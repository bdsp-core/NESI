...
#  - **Good neurological state (0):** ICANS(0)
#  - **Moderate state (1):** ICANS (1, 2)
#  - **Bad state (2):**  ICANS(3, 4)
# In[5]:
...
df_train = df_all_metadata[(df_all_metadata['Split'] == 'Train') & (df_all_metadata['Dataset'] == 'ICANS')].reset_index(drop=True)
df_val   = df_all_metadata[(df_all_metadata['Split'] == 'Val') & (df_all_metadata['Dataset'] == 'ICANS')].reset_index(drop=True)
df_test  = df_all_metadata[(df_all_metadata['Split'] == 'Test') & (df_all_metadata['Dataset'] == 'ICANS')].reset_index(drop=True)
...
# --------- LOAD MODEL ----------
model_path_trained = NSEI_ROOT / "Bespoke_models"/ "ModelCheckpoints_NEW" / "TripletCheckpoint" / "cohort_models" / "ICANS" / "ResNetGAP_ICANS_BestModel.pth"
...
model_bespoke_bdness_path = NSEI_ROOT / "Bespoke_models"/ "ModelCheckpoints_NEW" / "BespokeBadnessCheckpoint" / "cohort_models" / "ICANS" / "ICANS_Bespoke_Badness_Bet_model.pth"
train_model(
    model=model,
...
def get_scores(model, embeddings, device="cuda", batch_size=128):
    model.eval()
    model.to(device)
    # ---------------- SAFE CONVERSION ----------------
...
plot_score_distribution_by_raw(
    ...
    y_raw=Yraw_train,
    dataset_name="ICANS",
    title="ICANS Train Set Score Distribution"
)
