...
"""
Sedative infusion reconstruction (propofol, midazolam, ketamine,
dexmedetomidine) for the I0001 CAMS cohort. Mirrors build_sedative_exposure.py
but reads from I0001_CAMS_COHORT_MEDS_PARQUET and writes to
...
def process_drug(drug_name, parquet_path):
    ...
    out_path = med_config.I0001_CAMS_SEDATIVE_EXPOSURE_DIR / f"{drug_name}_intervals.csv"
...
def main():
    try:
        med_config.I0001_CAMS_SEDATIVE_EXPOSURE_DIR.mkdir(parents=True, exist_ok=True)
        parquet_path = str(med_config.I0001_CAMS_COHORT_MEDS_PARQUET).replace("\\", "/")
        print(f"Reading: {parquet_path}")
        print(f"Output:  {med_config.I0001_CAMS_SEDATIVE_EXPOSURE_DIR}")
