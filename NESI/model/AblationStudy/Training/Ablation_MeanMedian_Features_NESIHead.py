...
median_model_save_path = NESI_ROOT / "model" / "AblationStudy" / "ModelCheckpointsNew" / "Median_best_model.pth"
...
train_model(
    ...
    device=device,
    epochs=100,
    save_path=median_model_save_path,
    patience=20
)
...
train_scores_median_model = get_scores(model, X_train_data, device=device)
val_scores_median_model   = get_scores(model, X_val_data, device=device)
tst_scores_median_model  = get_scores(model, X_tst_data, device=device)
...
tst_scores_GCS = tst_scores[mask_GCS]
...
tst_scores_CAMS = tst_scores[mask_CAMS]
...
tst_scores_ICANS = tst_scores[mask_ICANS]
...
def plot_score_distributions_by_raw(
    ...
    for idx, d in enumerate(datasets):
        ...
        ax.set_ylabel("Predicted Badness Score", fontweight='bold', fontsize=12)
...
mean_model_save_path = NESI_ROOT / "model" / "AblationStudy" / "ModelCheckpointsNew" / "Mean_best_model.pth"
...
train_model(
    ...
    device=device,
    epochs=100,
    save_path=mean_model_save_path,
    patience=10
)
...
train_scores_mean_model = get_scores(model, X_train_data, device=device)
val_scores_mean_model   = get_scores(model, X_val_data, device=device)
tst_scores_mean_model  = get_scores(model, X_tst_data, device=device)
